...
(async () => {
  ...
  spotLight.target = sphere; // 光源跟随物体

  gui.add(sphere.position, "x").min(0).max(10).step(0.1);
  gui.add(sphere.position, "z").min(0).max(10).step(0.1);
  gui.add(spotLight, "intensity").min(0).max(10).step(0.1); // intensity - (可选参数) 光照强度。 缺省值 1。
  gui.add(spotLight, "distance").min(-10).max(100).step(0.1); // distance - 从光源发出光的最大距离，其强度根据光源的距离线性衰减。
  gui
    .add(spotLight, "angle")
    .min(0)
    .max(Math.PI / 2)
    .step(0.1); // angle - 光线散射角度，最大为Math.PI/2。
  gui.add(spotLight, "penumbra").min(0).max(1).step(0.01); // penumbra - 聚光锥的半影衰减百分比。在0和1之间的值。默认为0。
  gui.add(spotLight, "decay").min(0).max(1).step(0.01); // decay - 沿着光照距离的衰减量。需要开启 renderer.physicallyCorrectLights
